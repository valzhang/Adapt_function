function COM_FtCandle(hi, lo, cl, op, signal, dates)
...
% If the opening price is less than the close, box is emp
ca=nan(1, 2*length(signal));
ca(1) = 0;
%i=[];
for j=2:length(signal)
    if signal(j)-signal(j-1)>0 
        ca(2*j-1)= 1;
    elseif signal(j) - signal(j-1)<0 
        ca(2*j-1)=-1;
    else
        ca(2*j-1)=0;
    end
...
% 'edgecolor', 'r' 
% Modified by LiYang 2011/12/28
boxes(i) = patch([xbotpad(i)-0.5; xbotpad(i)-0.5; xtoppad(i)+0.5; xtoppad(i)+0.5],...
    [ytop(i)+2; ybottom(i)-2; ybottom(i)-2; ytop(i)+2], ...
    [zdata2(i); zdata2(i); zdata2(i); zdata2(i)], ...
    'r', 'edgecolor', 'r');
%% =========================================
...
% Modified by LiYang 2011/12/28
i=find(ca<0);
dboxes(i) = patch([xbotpad(i)-0.5; xbotpad(i)-0.5; xtoppad(i)+0.5; xtoppad(i)+0.5],...
    [ytop(i)+2; ybottom(i)-2; ybottom(i)-2; ytop(i)+2],...
    [zdata2(i); zdata2(i); zdata2(i); zdata2(i)], ...
    'g', 'edgecolor', 'g'); %#ok
...
%% =========================================
% 更改x轴为时间轴
newTick = linspace(1,length(dates),4)';
